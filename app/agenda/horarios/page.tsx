...
export default function HorariosPage() {
  ...
  const [horarios, setHorarios] = useState<HorarioSalao[]>(
  ...
  useEffect(() => {
    async function init() {
      ...
      const { data: horariosExistentes, error: horariosError } = await supabase
        .from('horarios_salao').select('*').eq('salao_id', salao.id)
      if (horariosError) setErro(mensagemErroAgenda(horariosError.message))
  ...
  function atualizar<K extends keyof HorarioSalao>(dia: number, campo: K, valor: HorarioSalao[K]) {
  ...
  async function salvar() {
    ...
    const { error: deleteError } = await supabase.from('horarios_salao').delete().eq('salao_id', salaoId)
    if (deleteError) {
      setErro(mensagemErroAgenda(deleteError.message))
      setSalvando(false)
      return
    }

    const { error: insertError } = await supabase.from('horarios_salao').insert(
      horarios.map(h => ({ ...h, salao_id: salaoId }))
    )
    if (insertError) {
      setErro(mensagemErroAgenda(insertError.message))
      setSalvando(false)
      return
    }
