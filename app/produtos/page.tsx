...
export default function ProdutosPage() {
  ...
  const [movimentando, setMovimentando] = useState<any | null>(null)
    ...
    <div style={{ minHeight: '100vh', background: t.bg, fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '36px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
          {[
            { label: 'Total produtos', value: produtos.length, unit: 'cadastrados' },
            { label: 'Em falta', value: emFalta.length, unit: 'abaixo do minimo', warn: emFalta.length > 0 },
            { label: 'Uso interno', value: produtos.filter(p => p.categoria === 'uso_interno').length, unit: 'produtos' },
            { label: 'Valor em estoque', value: `R$ ${valorEstoque.toFixed(0)}`, unit: 'revenda' },
          ].map(card => (
            <div key={card.label} style={{ background: (card as any).warn ? t.badgeInadimplente : t.bgCard, border: `0.5px solid ${(card as any).warn ? t.badgeInadimplenteText : t.borderCard}`, borderRadius: 16, padding: '18px 20px' }}>
              <p style={{ color: (card as any).warn ? t.badgeInadimplenteText : t.textFaint, fontSize: 10, letterSpacing: 2, textTransform: 'uppercase', margin: '0 0 8px' }}>{card.label}</p>
              <p style={{ color: (card as any).warn ? t.badgeInadimplenteText : t.text, fontSize: 28, fontWeight: 200, margin: '0 0 2px', lineHeight: 1 }}>{card.value}</p>
              <p style={{ color: (card as any).warn ? t.badgeInadimplenteText : t.textFaint, fontSize: 11, margin: 0 }}>{card.unit}</p>
            </div>
          ))}
        </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 14 }}>
                ...
                <div>
                  ...
                  <select value={categoria} onChange={e => setCategoria(e.target.value as any)} style={{ ...inputStyle, appearance: 'none' as const }}>
            ...
            return (
              <div key={p.id} style={{ padding: '14px 24px', borderBottom: i < filtrados.length - 1 ? `0.5px solid ${t.rowBorder}` : 'none', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {p.preco_custo && <p style={{ color: t.textFaint, fontSize: 11, margin: 0 }}>Custo: R$ {parseFloat(p.preco_custo).toFixed(2)}{p.preco_venda ? ` · Venda: R$ ${parseFloat(p.preco_venda).toFixed(2)}` : ''}</p>}
                </div>
