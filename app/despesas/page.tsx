...
export default function DespesasPage() {
  ...
  const totalMes = despesas.reduce((acc, d) => acc + parseFloat(d.valor), 0)
  const totalFixas = despesas.filter(d => d.tipo === 'fixa').reduce((acc, d) => acc + parseFloat(d.valor), 0)
  const totalVariaveis = despesas.filter(d => d.tipo === 'variavel').reduce((acc, d) => acc + parseFloat(d.valor), 0)

  const porCategoria = CATEGORIAS.map(cat => ({
    cat,
    total: despesas.filter(d => d.categoria === cat).reduce((acc, d) => acc + parseFloat(d.valor), 0)
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '36px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 20 }}>
          {[
            ...
            { label: 'Fixas', value: `R$ ${totalFixas.toLocaleString('pt-BR', { maximumFractionDigits: 0 })}`, unit: `${despesas.filter(d => d.tipo === 'fixa').length} lancamentos` },
            { label: 'Variaveis', value: `R$ ${totalVariaveis.toLocaleString('pt-BR', { maximumFractionDigits: 0 })}`, unit: `${despesas.filter(d => d.tipo === 'variavel').length} lancamentos` },
          ].map(card => (
            <div key={card.label} style={{ background: t.bgCard, border: `0.5px solid ${t.borderCard}`, borderRadius: 18, padding: '22px 26px', position: 'relative', overflow: 'hidden' }}>
              {(card as any).accent && <div style={{ position: 'absolute', top: 0, left: 0, width: 3, height: '100%', background: '#ef4444' }} />}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
                ...
                <div>
                  ...
                  <select value={tipo} onChange={e => setTipo(e.target.value as any)} style={{ ...inputStyle, appearance: 'none' as const }}>
          ...
          <div style={{ background: t.bgCard, border: `0.5px solid ${t.borderCard}`, borderRadius: 18, overflow: 'hidden' }}>
            ...
            {despesas.length === 0 ? (
              ...
              <div key={d.id} style={{ padding: '13px 24px', borderBottom: i < despesas.length - 1 ? `0.5px solid ${t.rowBorder}` : 'none', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <p style={{ color: '#ef4444', fontSize: 14, fontWeight: 400, margin: 0 }}>-R$ {parseFloat(d.valor).toLocaleString('pt-BR', { maximumFractionDigits: 0 })}</p>
