...
type AssinantePagamento = {
  id: string
  nome: string
  status: 'ativo' | 'cancelado' | 'inadimplente' | 'pausado' | string
  pacotes?: PacoteResumo | null
}
...
export default function PagamentosPage() {
  ...
  const [assinantes, setAssinantes] = useState<AssinantePagamento[]>([])
  ...
  const carregar = useCallback(async (id: string) => {
    ...
    setAssinantes((ass || []) as AssinantePagamento[])
  ...
  async function confirmarPagamento(assinante: AssinantePagamento) {
    if (!salaoId) return
  ...
  async function cobrarOnline(assinante: AssinantePagamento) {
    setProcessando(`online-${assinante.id}`)
    setErro('')

    const { data: { session } } = await supabase.auth.getSession()
    if (!session?.access_token) {
      setErro('Sessao expirada. Entre novamente para criar a cobranca online.')
      setProcessando(null)
      return
    }
    ...
    const res = await fetch('/api/stripe/checkout', {
      ...
      body: JSON.stringify({ assinanteId: assinante.id }),
    })

    const data = await res.json()
    if (!res.ok || !data.url) {
      setErro(data.error || 'Nao foi possivel criar o link de pagamento.')
      setProcessando(null)
      return
    }
  ...
  async function alterarStatus(assinante: AssinantePagamento, novoStatus: string) {
    if (!salaoId) return
    setProcessando(assinante.id)
  ...
  const pagStatusBg: Record<string, string> = { pago: t.badgeAtivo, pendente: t.badgeInadimplente, cancelado: t.badgeCancelado }
  const pagStatusText: Record<string, string> = { pago: t.badgeAtivoText, pendente: t.badgeInadimplenteText, cancelado: t.badgeCanceladoText }
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div>
                        ...
                        <p style={{ color: t.textFaint, fontSize: 11, margin: 0 }}>{a.pacotes?.nome} · R$ {Number(a.pacotes?.preco_mensal || 0).toFixed(0)}/mes</p>
                      </div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      {jaPagei ? (
                        ...
                        <>
                          <button onClick={() => cobrarOnline(a)} disabled={processando === `online-${a.id}`}
                            style={{ background: t.text, color: t.navBg, border: 'none', borderRadius: 8, padding: '7px 14px', fontSize: 11, cursor: 'pointer', opacity: processando === `online-${a.id}` ? 0.5 : 1 }}>
                            {processando === `online-${a.id}` ? 'Abrindo...' : 'Cobrar online'}
                          </button>
                          <button onClick={() => confirmarPagamento(a)} disabled={processando === a.id}
                            style={{ background: 'none', color: t.textMuted, border: `0.5px solid ${t.border}`, borderRadius: 8, padding: '7px 14px', fontSize: 11, cursor: 'pointer', opacity: processando === a.id ? 0.5 : 1 }}>
                            {processando === a.id ? 'Aguarde...' : 'Confirmar manual'}
                          </button>
                        </>
                      )}
                    </div>
          ...
          <div style={{ background: t.bgCard, border: `0.5px solid ${t.borderCard}`, borderRadius: 18, overflow: 'hidden' }}>
            {pagamentos.length === 0 ? (
              ...
              <div key={p.id} style={{ padding: '14px 24px', borderBottom: i < pagamentos.length - 1 ? `0.5px solid ${t.rowBorder}` : 'none', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <p style={{ color: t.text, fontSize: 13, margin: 0, fontWeight: 400 }}>R$ {Number(p.valor).toFixed(0)}</p>
